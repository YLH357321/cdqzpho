...
function generateDailyFortune(personName, birthDateStr) {
  ...
  // 注意：因为对比了三次，基础分可能会变高，这里假设 calculateThreePillarsRelation 返回的是平均分或总分
  // 如果返回的是总分(0-300)，这里可能需要除以3来保持量级，或者调整 dayFactor 的权重
...
function calculateThreePillarsRelation(current, birth) {
  ...
  pillars.forEach(p => {
    ...
    totalScore += res.score; // 假设原来的 score 是 0-100 或者类似的权重
